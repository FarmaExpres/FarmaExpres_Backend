...
test("GET /api/alerts/expiring-16-30-days returns expiring alerts between 16 and 30 days", async () => {
  ...
  try {
    const response = await new Promise((resolve, reject) => {
      http.get(
        `http://127.0.0.1:${address.port}/api/alerts/expiring-16-30-days`,
        (result) => {
          let body = "";
